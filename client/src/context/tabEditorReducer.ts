  ...
  switch (action.type) {
    ...
    case "ADD_MEASURE": {
      ...
      return {
        ...state,
        tab: newTab,
        position: { measure: measure === -1 ? 0 : measure, frame: 0 },
        isEditing: true,
      };
    }
    ...
    case "SET_POSITION": {
      ...
      if (isExistingPosition(tab, measure, frame)) {
        return { ...state, position: { measure, frame } };
      }

      if (frame >= tab[measure]?.length) {
        const nextMeasure = measure + 1;
        if (nextMeasure < tab.length) {
          return { ...state, position: { measure: nextMeasure, frame: 0 } };
        }
        return state;
      }
